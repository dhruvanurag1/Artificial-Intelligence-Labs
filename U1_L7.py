# Name: Dhruv Anurag       Data: 10/19/21
import random, pickle, math, time
from math import dist, pi, acos, sin, cos
from tkinter import *
...
def make_graph(nodes = "/Users/dhruvanurag/Documents/TJ/AI/rrNodes.txt", node_city = "/Users/dhruvanurag/Documents/TJ/AI/rrNodeCity.txt", edges = "/Users/dhruvanurag/Documents/TJ/AI/rrEdges.txt"):
 nodeLoc, nodeToCity, cityToNode, neighbors, edgeCost = {}, {}, {}, {}, {}
 map = {}   # have screen coordinate for each node location
 # Your code goes here
 with open(nodes, "r") as file1:
    for line1 in file1:
       line_array = line1.strip().split(" ")
       nodeLoc[line_array[0]] = [line_array[1], line_array[2]]
 with open(node_city, "r") as file2:
    for line2 in file2:
 ...
 with open(edges, "r") as file3:
      for line3 in file3:
         ...
         temp1 = nodeLoc[line_array[0]]
         temp2 = nodeLoc[line_array[1]]
         r1 = line_array[0]
         r2 = line_array[1]
         ...
         if r2 not in neighbors:
            ...
            neighbors[r2].add(r1)
         edgeCost[(r1, r2)] = calc_edge_cost(temp1[0], temp1[1], temp2[0], temp2[1])
         edgeCost[(r2, r1)] = calc_edge_cost(temp1[0], temp1[1], temp2[0], temp2[1])
 for a in neighbors:
    for b in neighbors[a]:
       edgeCost[(a, b)] = calc_edge_cost(nodeLoc[a][0], nodeLoc[a][1], nodeLoc[b][0], nodeLoc[b][1])
       edgeCost[(b, a)] = calc_edge_cost(nodeLoc[a][0], nodeLoc[a][1], nodeLoc[b][0], nodeLoc[b][1])
...
def bfs(start, goal, graph, col):
 ...
 while frontier:
    ...
    if s == goal:
       ...
       print("The number of explored nodes of BFS: ", explorednodes)
...
def bi_bfs(start, goal, graph, col):
 ...
 frontier, explored = [[start],[goal]], [{start: "s"},{goal:"s"}]
 while len(frontier[0]) > 0 and len(frontier[1]) > 0:
    s1 = frontier[0].pop(0)
    s2 = frontier[1].pop(0)
    if s1 in explored[1]:
       path1, cost1 = generate_path(s1, explored[0], graph)
       path2, cost2 = generate_path(s1, explored[1], graph)
       print("The number of explored nodes of Bi-BFS: ", explorednodes)
       print('The whole path: ', path1+path2)
       print("The length of the whole path: ", len(path1+path2))
       draw_final_path(ROOT, canvas, path1, graph)
       draw_final_path(ROOT, canvas, path2, graph)
       return path1+path2, cost1+cost2
    if s2 in explored[0]:
       path1, cost1 = generate_path(s2, explored[1], graph)
       path2, cost2 = generate_path(s2, explored[0], graph)
       print("The number of explored nodes of Bi-BFS: ", explorednodes)
       print('The whole path: ', path1+path2)
       print("The length of the whole path: ", len(path1+path2))
       draw_final_path(ROOT, canvas, path1, graph)
       draw_final_path(ROOT, canvas, path2, graph)
       return path1+path2, cost1+cost2
    for a1, a2 in ((x,y) for x in graph[3][s1] for y in graph[3][s2]):  #graph[3] is neighbors
       if a1 not in explored[0]:
          explored[0][a1] = s1
          frontier[0].append(a1)
          explorednodes += 1
          drawLine(canvas, *graph[5][s1], *graph[5][a1], col)
       if a2 not in explored[1]:
          explored[1][a2] = s2
          frontier[1].append(a2)
          explorednodes += 1
          drawLine(canvas, *graph[5][s2], *graph[5][a2], col)
    counter += 1
    if counter % 1000 == 0: ROOT.update()
 return None
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
 ...
 costs = dist_heuristic(start, goal, graph)
 explored = {start : (0, [start])}
 frontier.push((costs, start, [start]))
 while frontier.isEmpty() == False:
    a = frontier.pop()
    if a[1] == goal:
       path, cost = explored[a[1]][1], a[0]
       draw_final_path(ROOT, canvas, path, graph)
       print("The number of explored nodes of A Star: ", explorednodes)
       print('The whole path: ', path)
       print("The length of the whole path: ", len(path))
       return path, cost
    for i in graph[3][a[1]]:
       cost = explored[a[1]][0] + graph[4][(a[1], i)]
       if i not in explored or cost < explored[i][0]:
          cost2 = dist_heuristic(i, goal, graph)
          frontier.push((cost+cost2, i, a[2]+[i]))
          explored[i] = (cost, a[2]+[i])
          explorednodes += 1
          drawLine(canvas, *graph[5][a[1]], *graph[5][i], col)
    counter += 1
    if counter % 1000 == 0: ROOT.update()
def bi_a_star(start, goal, graph, col, heuristic=dist_heuristic):
 ...
 frontier[0].push((heuristic(start, goal, graph), start))
 frontier[1].push((heuristic(start, goal, graph), goal))
 usestartorgoal = (start, goal)
 exploredwithcostsandpath = [{start : (0, [start])},{goal : (0, [goal])}]
 sets = [{start}, {goal}]
 ...
 while not frontier[0].isEmpty() and not frontier[1].isEmpty():
    ...
    a = frontier[alternate].pop()[1]
    sets[alternate] = sets[alternate] - {a}
    if a in sets[1-alternate]:
       path, cost = exploredwithcostsandpath[alternate][a][1][:-1] + exploredwithcostsandpath[1-alternate][a][1][::-1], exploredwithcostsandpath[0][a][0] + exploredwithcostsandpath[1][a][0]
       draw_final_path(ROOT, canvas, path, graph)
       print("The number of explored nodes of Bi A Star: ", explorednodes)
       print('The whole path: ', path)
       print("The length of the whole path: ", len(path))
       return path, cost
    for i in graph[3][a]:
       currcost = exploredwithcostsandpath[alternate][a][0] + graph[4][(a, i)]
       if i not in exploredwithcostsandpath[alternate] or currcost < exploredwithcostsandpath[alternate][i][0]:
          explorednodes += 1
          cost2 = currcost + heuristic(i, usestartorgoal[1-alternate], graph)
          exploredwithcostsandpath[alternate][i] = (currcost, exploredwithcostsandpath[alternate][a][1] + [i])
          sets[alternate].add(i)
          frontier[alternate].push((cost2, i))
          drawLine(canvas, *graph[5][a], *graph[5][i], col)
    counter += 1
    if counter % 1000 == 0: ROOT.update()
 return None
def bi_a_star_helper(start, goal, graph, col, canvas, ROOT, heuristic=dist_heuristic):
 # Your code goes here
 counter = 0
...
def tri_directional(city1, city2, city3, graph, col, ROOT, canvas, heuristic=dist_heuristic):
 # Your code goes here
 path1, cost1 = bi_a_star_helper(city1, city2, graph, col, canvas, ROOT, heuristic)
 path2, cost2 = bi_a_star_helper(city2, city3, graph, col, canvas, ROOT, heuristic)
 path3, cost3 = bi_a_star_helper(city1, city3, graph, col, canvas, ROOT, heuristic)
 cost = min([cost1 + cost2, cost2 + cost3, cost1 + cost3])
 path = min([path1[:-1]+path2, path2[:-1]+path3, path3[:-1] + path1])
 draw_final_path(ROOT, canvas, path, graph)
 return path, cost
