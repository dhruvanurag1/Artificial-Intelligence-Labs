...
def utility(turn, tc, state):
   ...
   for a in tc:
      temp = {state[b] for b in a}
      if len(temp) == 1 and list(temp)[0] == turn: return 1
      elif len(temp) == 1 and list(temp)[0] != turn: return -1      
   return 0   
...
def max_value(state, turn, tc):
   ...
   v = (-10000000000000, state)
   for a, s in successors(state, turn):
      if (maximum:=(min_value(s, get_turn(s), tc)[0])) > v[0]: v = (maximum, s)
   return v
...
def min_value(state, turn, tc):
   ...
   v = (10000000000000, state)
   for a, s in successors(state, turn):
      if (minimum:=(max_value(s, get_turn(s), tc)[0])) < v[0]:
         v = (minimum, s)
   return v
